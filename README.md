...
4. Pick **background, effect and text colors** (or a palette swatch). Optionally **import a background image or video** (PNG/JPEG/WebP, or MP4/MOV/WebM). **Background** effects — tint, blur, brightness, saturation, grain, glitch, scanlines, chroma — hit only that picture; **Reactivity** lets them swing with the audio and **Punch** zooms the picture on bass. **Visualizer** effects — grain, bloom, glitch, scanlines, chroma, trail, reactivity — hit the audio graphics. With no imported background, the visualizer effects cover the whole frame. Jitter shakes the finished frame. Vignette darkens its edges.
5. Place up to **six text lines** anywhere on the frame. Drag a line in the preview, or set X, Y and size. The first two are title and subtext; the other four start off until you add them. Pick a **font** (Archivo, Bebas, metal, glitch, …) or **upload a TTF/OTF**. Font, color and text FX are shared. Optionally **upload a logo** (PNG with transparency) and place it above the title or in a corner. Text and logo have their own **glow / glitch / chroma / jitter**.
